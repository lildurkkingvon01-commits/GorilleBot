...
export async function getPlayersByGuild(guildId) { try { return await db.any(`SELECT id, discord_id AS "discordId", username, username AS "playerName", faction AS "playerFaction", url, guild_id AS "guildId", player_status AS "playerStatus", last_check_time AS "lastCheckTime", days_inactive AS "daysInactive", created_at AS "createdAt", updated_at AS "updatedAt" FROM players WHERE guild_id = $1`, [guildId]); } catch (e) { return []; } }
export async function getPlayerById(playerId) { try { return await db.oneOrNone(`SELECT id, discord_id AS "discordId", username, username AS "playerName", faction AS "playerFaction", url, guild_id AS "guildId", player_status AS "playerStatus", last_check_time AS "lastCheckTime", days_inactive AS "daysInactive", created_at AS "createdAt", updated_at AS "updatedAt" FROM players WHERE id = $1`, [playerId]); } catch (e) { return null; } }
...
export async function updateGuildConfig(guildId, updates) { try { const f = [], v = []; let p = 1; if (updates.inactivity_threshold) { f.push(`inactivity_threshold = $${p++}`); v.push(updates.inactivity_threshold); } if (updates.check_frequency) { f.push(`check_frequency = $${p++}`); v.push(updates.check_frequency); } if (updates.broadcast_channel_id !== undefined) { f.push(`broadcast_channel_id = $${p++}`); v.push(updates.broadcast_channel_id); } if (updates.command_permissions !== undefined) { f.push(`command_permissions = $${p++}`); v.push(updates.command_permissions); } if (f.length === 0) return null; v.push(guildId); const query = `UPDATE guild_configs SET ${f.join(", ")} WHERE guild_id = $${p} RETURNING *`; return await db.one(query, v); } catch (e) { console.error('[DB ERROR] updateGuildConfig:', e); return null; } }
